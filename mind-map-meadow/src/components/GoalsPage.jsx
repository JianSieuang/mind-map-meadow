...
export default function GoalsPage({ tasks, onAddTask, onEditTask, onDeleteTask, onCompleteTask }) {
    ...
    const oneOffTasks = tasks.filter((t) => !t.recurring_task_id);
    const pendingAll = [...oneOffTasks.filter((t) => t.status === "pending")].sort((a, b) => new Date(a.due_date) - new Date(b.due_date));
    const completedAll = [...oneOffTasks.filter((t) => t.status === "completed")].sort((a, b) => new Date(b.completed_at) - new Date(a.completed_at));
            ...
            <div className="flex flex-col gap-2">
                {visibleTasks.map((task) => {
                    if (activeTab === "completed") {
                        return (
                            <div key={task.id} className="bg-[#23232e] p-3 rounded-lg flex items-center justify-between gap-3 border-l-4 border-emerald-600 border-y border-r border-slate-700/40 opacity-80">
                                ...
                                {confirmDeleteId === task.id ? (
                                    <button onClick={() => handleDelete(task.id)} className="bg-rose-600 hover:bg-rose-500 text-white font-bold py-1 px-2 rounded text-[11px] transition shrink-0">
                                        Confirm?
                                    </button>
                                ) : (
                                    <button onClick={() => setConfirmDeleteId(task.id)} className="bg-slate-600 hover:bg-slate-500 text-white font-bold py-1 px-2 rounded text-[11px] transition shrink-0">
                                        🗑️
                                    </button>
                                )}
                            </div>
